...
namespace Sichem
{
    internal class ConversionProcessor : BaseProcessor
    {
        ...
        private class ProcessingState
        {
            ...
            public ProcessingState(State type, string name)
            {
        ...
        private Stack<ProcessingState> _stateStack = new Stack<ProcessingState>();

        protected override TextWriter Writer => _stateStack.Peek().Writer;
        ...
        {
            _stateStack.Push(new ProcessingState(state, name));
        }
        ...
        private void EndState()
        {
            ...
            var builder = state.Writer.GetStringBuilder();
            string name = state.Name;

            switch (state.Type)
            {
                case State.Structs:
                    _result.Structs[name] = builder;
                    break;
                case State.GlobalVariables:
                    _result.GlobalVariables[name] = builder;
                    break;
                case State.Enums:
                    _result.Enums[name] = builder;
                    break;
                case State.Constants:
                    _result.Constants[name] = builder;
                    break;
                case State.Functions:
                    _result.Methods[name] = builder;
                    break;
            }
        }
